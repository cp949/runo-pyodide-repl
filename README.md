...
상태: 구현 초기. `ROADMAP.md` 순서로 진행하며 RD-001(워크스페이스 정비, xterm-readline 벤더링)·RD-002(프로토콜 코어: RPC, stdin 메일박스, interrupt buffer, 초기화 프레임)·RD-003(터미널 마운트와 줄 편집)·RD-004(pyodide 로드, 배너, 출력 sink 4종, 전역 스트림)·RD-005(REPL 루프와 PyodideConsole 코어)까지 끝났다. 데모는 pyodide를 CDN에서 worker로 로드해 배너를 낸 뒤 한 줄 REPL로 동작한다: `>>> ` 프롬프트에서 식의 값 에코, 트레이스백·SyntaxError 표시, 미완성 블록(`... `), 개행 없는 출력 뒤 `t>>> ` 이어붙임, `exit()`로 세션 종료(상태 `terminated`)를 지원하고, `crossOriginIsolated`와 세션 상태(`loading`·`ready`·`load-failed`·`not-isolated`·`terminated`)를 표시한다. 개행이 든 붙여넣기·Shift+Enter 제출은 아직 분할되지 않아(RD-011) 통째로 실행되고 대개 SyntaxError가 난다. `input()`·Ctrl+C·세션 리셋도 후속 RD다.
