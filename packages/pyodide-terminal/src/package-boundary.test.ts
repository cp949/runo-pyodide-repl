...
/**
 * terminal 패키지 경계: 소비자에게 가는 런타임 의존 트리(작업공간 내부 core·xterm-readline과 그 전이 의존 포함)에 동기 브리지
 * 라이브러리(`coincident`·`reflected-ffi`, ADR-0001)가 없다. "coincident 없이 실행창을 쓴다"는 약속을 강제한다.
 */
