...
/**
 * react 패키지 경계: 소비자에게 가는 런타임 의존 트리(작업공간 내부 core·terminal·repl과 그 전이 의존 포함)에 동기 브리지
 * 라이브러리(`coincident`·`reflected-ffi`, ADR-0001)가 없다. "coincident 없이 React 컴포넌트를 쓴다"는 약속을 강제한다.
 */
...
describe("react 패키지 의존 트리", () => {
  ...
  test("작업공간 내부 패키지와 그 전이 의존까지 실제로 따라갔다", () => {
    ...
    expect(names).toContain("@cp949/runo-pyodide-terminal");
    expect(names).toContain("@cp949/runo-pyodide-repl");
    expect(names).toContain("@cp949/runo-xterm-readline");
    expect(names).toContain("string-width");
    expect(names).toContain("@xterm/addon-fit");
