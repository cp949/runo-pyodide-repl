...
**깨우기**:
정지한 실행을 취소해 사용자 지점에서 `KeyboardInterrupt`가 나게 하는 것. Python `interrupt_idle()`(TS 타입 `InterruptIdle`)이 진입점이고 감시 타이머와 핸들러 규칙 ③이 부른다. 깨웠으면 참을 돌려주지만 **거짓이 "깨우지 못했다"를 뜻하지는 않는다**(핸들러 규칙 ③이 한 틱 미뤄 둔 깨우기가 먼저 돈 경우가 있다).
_Avoid_: 인터럽트, 재개
