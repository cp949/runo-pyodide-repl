...
describe("createReadGuard: stdin 읽기는 활성 REPL 읽기가 끝난 뒤에 시작한다", () => {
  test("활성 REPL 읽기가 있으면 stdin 읽기는 그 읽기가 끝난 뒤에 시작하고 두 결과는 그대로 전달된다", async () => {
    ...
    const repl = guard.readLine(">>> ", undefined, true);
  ...
  test("REPL 읽기는 가드를 거쳐도 즉시 시작된다(시작 타이밍 불변)", () => {
    ...
    expect(rawReadLine).toHaveBeenCalledWith(">>> ", undefined, true);
  });
  ...
  test("`cancelable` 인자를 그대로 원본 읽기에 넘긴다", async () => {
    ...
    void guard.readLine(">>> ", undefined, false);
    void guard.readInput(false);
    // stdin 읽기는 활성 REPL 읽기가 끝난 뒤에 시작한다.
    replReadAt(0).resolve("x = 1");
    await tick();

    expect(rawReadLine).toHaveBeenCalledWith(">>> ", undefined, false);
    expect(rawReadInput).toHaveBeenCalledWith(false);
  });

  test("`pending`을 그대로 원본 REPL 읽기에 넘긴다", async () => {
    const { guard, rawReadLine, replReadAt } = createGuarded();

    void guard.readLine("... ", "for i in range(2):", true);
    replReadAt(0).resolve("    print(i)");
    await tick();
