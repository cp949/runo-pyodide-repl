 ...
 *
 * 읽기를 시작하며 꼬리를 비운다: 읽는 동안 온 배경 출력(asyncio task·`call_later` 콜백·전역 스트림)은 sink가 벤더 접두로
 * 관리하고 꼬리 추적기에 먹이지 않으며(05-output.md, RD-022b), 읽기가 끝나면(Enter, Ctrl+C 취소) 커서가 다음 행 처음으로 가므로
 * 꼬리가 없다. 비우지 않으면 다음 읽기가 앞 꼬리를 물려받는다.
 *
