 ...
 * 유지한다. 바이트 열이 아니라 결과 화면을 단정하려는 시험(`run-source.test.ts`)이 쓴다. 벤더 `xterm-readline`의 `VTerm`과 같은 모델을
 * 이 패키지 시험용으로 줄여 둔 것이다(패키지 경계 때문에 벤더 시험 도구를 가져오지 않는다). VT 전체를 해석하지 않는다.
 */
