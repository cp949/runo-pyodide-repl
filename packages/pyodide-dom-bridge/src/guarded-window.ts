 ...
 * 던진다. Python `JsProxy`가 속성을 읽을 때 이 요청이 나가므로 원격 프록시를 target으로 삼거나 그대로 노출하면 `window.document`가
 * Python에서 실패한다(Chromium L1 실측). 그래서 모든 연산을 원격으로 직접 위임하고, 설명자는 원격 설명자를 묻지 않고 값에서 만들어
 * 설정 가능(configurable)한 데이터 설명자로 돌려준다(빈 target에는 없는 속성이라 불변식에 걸리지 않는다).
 */
