 ...
 *
 * 이 파일은 `protocol/`을 import하지 않는다. `ack`·`seq`·`discard`는 `boot.ts`가 core 프로토콜의 함수를 클로저로 넣는다
 ...
 * 돌려주는 `interrupt_idle`은 감시 타이머가 쓰고, 호출자가 세션 끝에 destroy한다. 조각 교체·정지한 실행 깨우기·트레이스백
 * 파일명 가드가 기대와 다르면 `deps.report`로 알린다(`sleep-slice`·`run-sync`·`webloop-filename`, 경고는 main이 한 번 낸다).
 */
export function connectInterrupts(
  ...
  const codes = installSleepSlice(pyodide, { report: deps.report });
