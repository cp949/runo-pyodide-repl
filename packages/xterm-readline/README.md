...
| export 추가    | `src/index.ts`(신규)                                                    | `Readline`, `History`, `InputType`, `State`, `Tty`와 타입 `HistoryOptions`·`ReadlineOptions`. 원본 파일에 export를 덧붙이지 않는다                                                                                                                                                                                                                          |
| 시험 이식      | `src/*.test.ts` 8개                                                     | jest → vitest. `vitest` import 추가, `jest.fn` → `vi.fn`(`state`·`tty`). 시험 본문은 그대로                                                                                                                                                                                                                                                                 |
| 시험 추가      | `src/persist.test.ts`, `src/index.test.ts`, `src/dispose.test.ts`(신규) | `persist` 옵션, 공개 export 표면, `dispose` 정책(write 콜백 동기/비동기)                                                                                                                                                                                                                                                                                    |
