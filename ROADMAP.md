...
상태: 구현 완료 — 리뷰·병합 대기(2026-09-24) · 이전: RD-022a · 설계: `05-output.md` 4.4, `04-stdin-input.md` 3.2·3.3, `06-editing.md` 6.1, `02-console-core.md` 5.6.3·5.6.7, `10-parity-deviations.md` 편차 4·54·55
...
- 미수행: L2·L3, 브라우저 RED(RED는 L0 jsdom), preview 서버 실행(dev 전용 셀). 재그룹화·`dev` 병합은 이 기록 뒤 단계다.
...
- 후속 이슈(전부 `deferred`): `.scratch/repl-run-source-followups/issues/`의 09(그리기 전 창의 개행 없는 조각 유실, 확정 3 범위 제외)·10(재그리기 대기 중 리사이즈, 공개 편집 API 부분은 리뷰 반영으로 고침)·11(`run-source-check`가 `run.mjs` `SETS`에 없음)·12(배경 `input()`의 두 알림 사이 Enter 경합)·13(재그리기 콜백 전 `cancelRead()`의 미그림 접두 유실)·14(출력으로 커지는 접두의 뷰포트 초과·초선형 쓰기량)·15(접두 안 BS·BEL). `.scratch/sigint-test-isolation/issues/05`(`sigint-handler-idle` `__subclasscheck__`/`ConversionError` 모양 1회), 04에 `--concurrency=1` 1회차 관찰 추가. 이슈 07·02는 `done`, 05에 화면 모델 이동 기록.

인계: 소비자가 쓸 새 공개 API는 벤더 `@cp949/runo-xterm-readline`의 `Readline.isReading()`·`abovePrefix()`·`printAboveRaw(lines, prefix)`와 `State.setPromptPrefix()`·`promptPrefix()`다(계약은 `06-editing.md` 6.1). repl 공개 export는 바뀌지 않았다. 재그리기 대기 중 공개 편집 API는 버퍼만 고치고 콜백이 편집 뒤 커서로 그리므로 새 소비자(RD-024 React 래퍼 등)가 불러도 된다. 재그리기 대기 중 리사이즈는 입력줄 흔적을 남길 수 있다(이슈 10). 확인 도구는 저장소에 있다: `apps/demo/e2e/checks/bg-output-check.mjs`(`e2e:bg-output`), `apps/demo/e2e/lib.mjs`의 `installRpcTap`·`rpcNoticeCount`·`injectRpcNotice`(화면 신호 없는 제품 이벤트 대기, main 쪽 알림 합성), `@repo/pyodide-testkit/vt-screen`. 실행 로그·변이 검사기(`mutate-safe.mjs`, 스펙 `mutations-delta0{1,2,3}*.json`·`mutations-delta05b-*.json`, 생성기 `gen-mutations-delta0{1,2,3}.mjs`·`gen-mutations-delta05b.mjs`)·조사 프로브(`probe-07.test.ts`)·제목 대조 결과는 `_works/_completed/20260924-28-rd-022b-bg-output-above-read/verify/`에 있다. 함정: `docs/traps/TRP-058`~`TRP-060`.
