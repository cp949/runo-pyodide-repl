...
- 12: REPL 줄이 열린 채 배경 `input("bg> ")`이 오고 `write`·`readInput` 두 알림 사이에 Enter를 치면 stdin 입력 행에 프롬프트가 없다.

완료 기준:

- 각 이슈 "완료 기준"의 jsdom 시험이 기대값으로 통과한다: 15(벤더 폭 계산 2재현 + `\r` 예외 경로), 16(제출 `impx`·`imp`, 배경 출력 없는 대조 2개 유지), 10(`> abcdefgh` 1번·콜백 뒤 커서 복원), 12(stdin 입력 행 `bg> hello`).
- 4건 각각 구현 전 RED를 확인하고, 구현을 뒤집는 변이 검사로 시험 검출력을 확인한다.
- L1: 15·10은 브라우저 확인 셀을 새로 만들어 수정 전 실패·수정 후 통과를 본다(`apps/demo/e2e/checks/`, RD-018 확정 11). 16·12는 브라우저 관찰을 완료 기준으로 삼지 않는다(jsdom RED로 판정).
- 기존 L1 중 변경 영역(`repl-check`·`stdin-input`·`prompt-cancel`·`run-source`·`react-fit`)이 `BASELINE.md`와 같은 개수·`pageErrors`다. L2는 사용자 지시 때만.
- 이슈 13(`cancelRead()`가 미그린 접두를 잃는다)은 같은 하니스로 판정해 수정 또는 `05-output.md` 4.4 문서화 유지 중 하나로 결론을 적는다.
- 이슈 09·14·04는 범위 밖이다(`deferred` 유지, 사유는 각 이슈 Comments).

L1 예산: 새 셀 2종 × (양성 대조 1 + 통과 1) + 기존 5종 × 1 = 9회.
