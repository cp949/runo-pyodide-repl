...
- L1(dev, Chromium): **사용 14회/14회, 사용자가 상한을 10회에서 14회로 확장했다**(DELTA-02 5회: 회귀 4종 + URL 인자 누락 명령 오류 재실행 1, DELTA-04 9회: e2e 중단 1·진단 프로브 3·재개 e2e 1·전체 재실행 1·양성 대조 1·`react-strictmode` 1·`ONLY=S6` 재실행 1). L2·L3 미실행.
  ...
  - 신규 `e2e:dom-bridge` **24/24(한계 있음)**, 7페이지: 전체 재실행에서 22셀 통과(초기 격리·`native` true, S1 `ready`·`print(1+1)`·worker 1개, S2 `document.title`·canvas 픽셀·guarded `window`, S3 `input()`(`hello 한글`)·취소 2종·재입력, S4 `while True`·`time.sleep(10)` Ctrl+C·이후 실행, S5 `interrupt()` 결말이 호출 반환 뒤·다음 줄 `interrupted`·`stop()` → `restarted`·새 `ready`·옛 slow 종료 뒤 pageerror 0, S7 전역 패치 뒤 core 채널, `?native=0` main·worker 양쪽 `load-failed`+명시 문구(N0a·N0b), 늦은 dom-bridge import → `load-failed`+첫 정적 import 문구, 늦은 `runWorker` → `ready`, 끝 콘솔·pageerror 0). S6 2셀은 아래 예외대로 저장된 결과의 재판정이다. 양성 대조(브라우저 1건): `prepare`의 부트스트랩 검사를 제거하면 `ONLY=LATE`가 실패한다(`load-failed`가 오지 않고 `loading`에 머문다, 원복 확인). `pageErrors` 0.
- 예외·정직한 한계:
  ...
  2. **역전 수의 원인은 확인하지 않았다.** core 직접 C 0~5/100·view C 60~90/100(60·90·60·69)·Ag·Ar 0/100. 스파이크의 core 직접 0/500과 이번 5/100의 차이도 원인 미확인이다(view 경로 가설: terminal 출력 렌더 지연, 검증하지 않았다). 문서는 "순서를 보장하지 않는다"로 적고 "0을 보장한다"고 쓰지 않는다. 후속 이슈 `dom-bridge-followups/03`(`deferred`).
