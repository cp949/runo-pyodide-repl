...
- L1(dev, 각 1회): `bg-output` 기본 10/10(B08 통과 포함: 커서 열 14), `repl-check`(normal) 15/15, `stdin-input` 19/19, `prompt-cancel` 23/23, `run-source` 12/12, `react-fit` 12/12. `pageErrors` 0, `problemLogs` 0, `BASELINE.md`와 같은 개수(기대값이 바뀐 행 없음). B08 양성 대조는 수정 전 코드에서 열 14 대기가 시간 초과로 실패했다. 누적 L1 9회, 예산 안. L3 미실행.
- L2(2026-09-25, 병합 뒤 사용자 지시로 1회): `pnpm --filter demo e2e:baseline` — 스크립트 47개 전부 통과(실패 0건), 초과 `pageErrors` 0(`baseline.json`의 `expectedPageErrors` 3건은 `session-reset` `crash` 절·`tla` `sticky` 절의 의도한 `Error: forced`), `summary.json` `ok=true`, `deviations` `["AD"]`(기준선과 같음)·`unrun` 없음. 47개 셀 전부 `BASELINE.md`와 같은 개수다(기대값이 바뀐 행 없음). 실행 전 `pnpm build`는 7/7 캐시 적중이었다.
