...
- 후속 이슈(`.scratch/`): `dom-bridge-followups/`(신규) 01 CSP 정적 검사 주석 제거가 정규식(`deferred`)·02 `smoke:pack` 진입점 손동기화 세 곳(`deferred`)·03 출력·DOM 도착 순서 역전 원인 미확인(`deferred`), `sigint-test-isolation/` 06 `sigint-handler.test.ts` 눌림 20회 `19\n` 간헐 실패(`deferred`)·07 core `runner-pyodide.test.ts` `stop()` 폴백 `stopped` 1회 실패(`deferred`), 기존 04(sleep-slice 병렬 부하 재현율 추가 관찰)·05(`__subclasscheck__` + `ConversionError` 모양 재관찰로 `open` 전환) Comments 보강, `react-package-followups/05` `done`. 함정: 스파이크에서 승격한 `TRP-049`(worker 수 판정 조건 보강)·`TRP-065`~`067`, 이 RD에서 승격한 `TRP-068`~`072`(Vite dev의 `development` 대상 누락 해석·worker 리스너 등록 순서와 coincident 부트스트랩 삼킴·`sideEffects: false`의 부수효과 전용 import 제거·원격 window 프록시 감싸기의 비설정 속성 불변식·worker 전용 bare import 첫 요청 시 전체 다시 불러오기).
