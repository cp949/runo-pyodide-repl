...
- RD-022b가 소스에 더한 공개 API(terminal sink가 쓴다, `05-output.md` 4.4): `Readline.isReading(): boolean`, `Readline.abovePrefix(): string`, `Readline.printAboveRaw(lines: string, prefix: string): Promise<void>`, `State.setPromptPrefix(prefix: string): void`·`State.promptPrefix(): string`. `takeRead()` 반환 형태는 그대로다.
  ...
  - 시험은 `print-above-raw.test.ts`(45개, `StubTerminal` + `VTerm`: `isReading`·`abovePrefix` 수명, 지우기·쓰기·다시 그리기, 재그리기 대기, 재그리기 대기 중 공개 편집 API 11개, 읽기 밖, Tab `printAbove`와 접두·이슈 02 재현)와 repl `run-source.test.ts` "Tab 완성 응답과 배경 출력 재그리기의 겹침"(응답이 콜백보다 먼저·대조 2개). 기존 `print-above.test.ts`·`take-read.test.ts`·`type-ahead.test.ts`·`cancel.test.ts`는 제목·단정 변경 없이 통과한다. 변경 목록은 `packages/xterm-readline/README.md`.
