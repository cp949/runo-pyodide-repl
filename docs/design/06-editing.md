...
- RD-022b가 소스에 더한 공개 API(terminal sink가 쓴다, `05-output.md` 4.4): `Readline.isReading(): boolean`, `Readline.abovePrefix(): string`, `Readline.printAboveRaw(lines: string, prefix: string): Promise<void>`, `State.setPromptPrefix(prefix: string): void`·`State.promptPrefix(): string`. `takeRead()` 반환 형태는 그대로다.
  ...
  - **알려진 경계**: `onResize`의 `state.refresh()`는 재그리기 대기 중에도 곧바로 그린다 — 레이아웃이 초기화된 상태라 출력 아래에 입력줄을 먼저 그리고 콜백이 다시 그려 감긴 입력은 첫 행이 흔적으로 남는다(jsdom 재현: cols 10 `abcdefghijkl` → `printAboveRaw("t\n", "")` 콜백 전 resize → `> abcdefgh` 2번, `.scratch/repl-run-source-followups/issues/10-*.md` `deferred`). 재그리기 대기 중 `cancelRead()`는 아직 그리지 않은 접두를 쓰지 않는다(`05-output.md` 4.4, `.scratch/repl-run-source-followups/issues/13-*.md`). 재그리기 대기 중 `queued`에 쌓인 키는 버퍼에 아직 없어 공개 편집 API 호출자의 경합 판정(`getLine()`·`getCursor()`)에 보이지 않고, 오프스크린 편집은 곧바로 버퍼에 들어가며 큐의 키는 콜백에서 뒤에 재생된다 — 도착 순서(키 → 편집)가 뒤집힌다. Tab 완성에서 `imp` → Tab → 배경 출력 → `x` → 완성 응답이면 `importx`(배경 출력이 없으면 `impx`)가 제출된다(`07-tab-completion.md` 7.3, `.scratch/repl-run-source-followups/issues/16-*.md` `deferred`).
  - 시험은 `print-above-raw.test.ts`(46개, `StubTerminal` + `VTerm`: `isReading`·`abovePrefix` 수명, 지우기·쓰기·다시 그리기, 재그리기 대기, 재그리기 대기 중 공개 편집 API 12개, 읽기 밖, Tab `printAbove`와 접두·이슈 02 재현)와 repl `run-source.test.ts` "Tab 완성 응답과 배경 출력 재그리기의 겹침"(응답이 콜백보다 먼저·대조 2개). 기존 `print-above.test.ts`·`take-read.test.ts`·`type-ahead.test.ts`·`cancel.test.ts`는 제목·단정 변경 없이 통과한다. 변경 목록은 `packages/xterm-readline/README.md`.
