...
- RD-022b가 소스에 더한 공개 API(terminal sink가 쓴다, `05-output.md` 4.4): `Readline.isReading(): boolean`, `Readline.abovePrefix(): string`, `Readline.printAboveRaw(lines: string, prefix: string): Promise<void>`, `State.setPromptPrefix(prefix: string): void`·`State.promptPrefix(): string`. `takeRead()` 반환 형태는 그대로다. RD-026이 두 개를 더했다(아래): `Readline.undrawnAbovePrefix(): string`, `Readline.hasQueuedInput(): boolean`.
  ...
  - **`undrawnAbovePrefix()`**(RD-026): `abovePrefix()` 중 아직 화면에 그리지 않은 것. 재그리기(`printAboveRaw`)의 write 콜백을 기다리는 동안(`redrawing`)에만 `abovePrefix()`를, 그 밖에는 `""`를 돌려준다. 재그리기 대기 중에는 입력줄이 접두째 지워져 접두가 화면에 없고, 콜백 뒤에는 접두가 프롬프트 행에 이미 그려져 있다. `cancelRead()`는 화면에 쓰지 않으므로 콜백 전에 취소하면 접두가 사라지고, 취소 앞에서 이 값을 읽어 다시 쓰는 것은 호출자 몫이다(reset·실행창 abort, `05-output.md` 4.4). `abovePrefix()`를 그대로 다시 쓰면 재그리기가 끝난 뒤 취소에서 접두가 두 번 나온다(`tick>>> pritick`). 벤더 계약(`cancelRead()`는 화면에 아무것도 쓰지 않는다)은 바뀌지 않는다.
  ...
  - **재그리기 대기 중 리사이즈·취소·큐**(RD-026): (1) `onResize`는 크기(`tty.col`·`tty.row`·`anchorRow`)를 갱신하되 재그리기 대기 중(`redrawing`)이면 `state.refresh()`를 생략한다 — 입력줄이 화면에 없는데 지금 그리면 출력 아래에 흔적 행이 남고(감긴 입력이면 첫 행이 흔적) 콜백이 한 번 더 그린다. 콜백(`finishRedraw`)의 `resetLayout()` + `refresh()`가 새 크기로 그린다. 같은 가드가 재그리기 대기 중 리사이즈 뒤 `takeRead()`에 입력줄 잔여가 남는 것도 없앤다. (2) 콜백 전 `cancelRead()`가 아직 그리지 않은 접두를 잃는 문제는 벤더 계약을 바꾸지 않고 호출자가 `undrawnAbovePrefix()`로 복원한다(`05-output.md` 4.4). (3) 재그리기 대기 중 `queued`에 쌓인 키는 버퍼에 아직 없으므로 경합 판정 호출자는 `hasQueuedInput()`이 참이면 편집을 하지 않는다(`07-tab-completion.md` 7.3). 오프스크린 편집은 곧바로 버퍼에 들어가고 큐의 키는 콜백에서 뒤에 재생돼 도착 순서(키 → 편집)가 뒤집히므로, 호출자가 이 값을 보지 않으면 `imp` → Tab → 배경 출력 → `x` → 완성 응답에서 `importx`가 제출된다.
