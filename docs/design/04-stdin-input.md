...
- **경합: 두 알림 사이의 Enter**(RD-022b 리뷰, jsdom 재현; 고치지 않고 편차 56으로 등록, RD-026 사용자 확정 2026-09-25): worker는 `write("bg> ")`와 `readInput` 알림을 같은 포트로 연달아 보낸다. 둘 사이(다음 메시지 태스크 한 번)에 REPL 줄 Enter가 처리되면 읽기가 끝나 `abovePrefix()`가 `""`이고, 접두 `bg> `는 확정된 행(`bg> >>> x = 41`)에만 남고 꼬리는 비어 있어(열린 읽기 출력은 꼬리에 먹이지 않는다) stdin 읽기가 프롬프트 없이 열린다: 화면 `["bg> >>> x = 41", "hello"]`, 경합 없는 경로는 `[">>> x = 41", "bg> hello"]`. 원인은 `inputDeferred` 호출 조건이다: Enter 처리의 마이크로태스크가 `readInput` 메시지 태스크보다 먼저 돌아 도착 시 `replOpen`이 이미 false라 `inputDeferred`가 불리지 않는다(위 불릿의 "활성 REPL 읽기가 있을 때"). 벤더가 Enter 시점 접두를 보관해 `moveAbovePrefixToTail`이 폴백으로 꼬리를 채우는 방식을 구현·시험했으나 포기했다: 폴백에 닿으려면 호출 조건을 풀어야 하는데 그러면 접두가 남은 채 `input()` 코드를 Enter로 제출하는 정상 경로에서 접두가 stdin 프롬프트로 재그려지거나 `input("name: ")` 프롬프트를 덮을 수 있고(추정, 미시험), 기존 read-guard 시험 2건("활성 REPL 읽기가 없으면 …", "끝난 REPL 읽기 뒤에 온 readInput은 …")의 기대값을 바꿔야 한다. 경합과 정상 지연을 main이 구분할 수단이 없다. 재개 조건은 그 구분 수단이 생길 때다(`10-parity-deviations.md` 편차 56, `.scratch/repl-run-source-followups/issues/12-*.md`). RD-022b 전에는 sink가 읽기 중 출력도 꼬리에 먹여 이 경합에서도 `bg> `가 stdin 프롬프트가 됐다(코드 읽기).
...
- 색: 줄 경계를 넘어 열린 SGR 시퀀스를 꼬리 앞에 이어 붙인다(시퀀스 목록으로 보관, 첫 파라미터가 0이거나
  ...
  SGR 외 CSI(이동·지우기·`\x1b[?25l` 등)와 OSC는 걸러내지 않고 통과시킨다. 커서를 옮기지 않고 글자만 바꾸는 제어 문자는 정규화한다:
  BS(`\b`)는 본문 마지막 글자를 지우고, BEL 등 나머지 C0와 DEL은 제거한다(`05-output.md` 4.4 "접두 안의 제어 문자와 폭"; 접두와
  `input()` 프롬프트 꼬리가 같은 추적기를 쓴다). 단 문자열 시퀀스(OSC `\x1b]`·DCS 등) 안은 종료자(BEL·ST)까지 손대지 않는다 —
  종료자를 지우면 시퀀스가 열린 채 남아 터미널이 뒤따르는 프롬프트까지 삼킨다.
...
- stdin 경로(`packages/pyodide-terminal/src/stdin-reader.ts`의 `createInputReader(readline, term, sinks)` → `read(cancelable): Promise<string | null>`, 세션마다 하나)는 꼬리 그대로가
  ...
  REPL 리더와 같은 "`dispose()` 뒤 write 콜백을 전달하지 않는 터미널 뷰"(surface `openIo()`의 `io.terminal`, `packages/pyodide-terminal/src/surface.ts`, TRP-004)를 준다. 세션마다 새 `io`를 열고 `io.close()`는 core 세션의 `terminate` 훅이 부른다.
