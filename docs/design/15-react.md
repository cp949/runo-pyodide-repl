...
- L0(`pnpm --filter @cp949/runo-pyodide-react test`, jsdom): 111개 = 경계 2(`package-boundary.test.ts`) + `use-python-runner.test.tsx` 26 + `python-runner.test.tsx` 44 + `python-repl.test.tsx` 39(RD-024 병합 때 107개, 사후 리뷰에서 4개 추가: hook 생성 옵션 전달, `PythonRunner` 생성 실패 시 `Terminal` 정리, rAF 콜백의 실행 시점 크기 재확인, `ResizeObserver` 없을 때 마운트 1회 fit). 가짜 `Worker`(`src/test-utils/fake-worker.ts`)와 실제 `createRunner`·`createTerminalRunner`·`createRepl`·`@xterm/xterm`을 쓴다. 확인 항목: StrictMode worker·`.xterm` 수, 언마운트 뒤 worker 0·`Terminal.dispose` 호출, 인라인 콜백 재렌더에서 worker·`Terminal` 수 불변, 핸들 위임 규칙(15.3), `copyOnSelect` 토글, fit(가짜 `ResizeObserver`·rAF로 크기 0 가드·통지 합침·`fit={false}`), 정리 순서(`Terminal.prototype.dispose` 시점의 live worker 수 `[0, 0]`), 타입 유도(`expectTypeOf`).
- 변이 검사: `usePythonRunner` 20개 중 19 killed·1 동등, `PythonRunner` 40개 중 38 killed·2 동등, `PythonRepl` 31개 중 30 killed·1 동등. 살아남은 4건은 다른 경로가 같은 결과를 만들어 관찰할 수 없는 변이다: cleanup에서 핸들 참조를 비우지 않기(dispose된 핸들이 inert), 생성 때 `copyOnSelect` 옵션 전달 제거(마운트 직후 effect가 같은 값을 다시 설정, 두 컴포넌트), `status`가 항상 마지막 통지 값을 돌려주기(core `status`와 마지막 통지가 항상 같음). 정리 순서를 뒤집는 변이는 콘솔 경고 단언으로는 잡히지 않고 `dispose` 시점의 live worker 수 단언으로 잡힌다(`docs/traps/TRP-064`). 사후 리뷰(2026-09-25)에서 위 목록에 없던 변이 4개(hook의 `filename` 전달 제거, `PythonRunner` 생성 실패 `catch`의 `view.dispose()` 제거, 크기 0 가드를 rAF 콜백에서 통지 시점으로 옮김, 마운트 1회 fit을 `ResizeObserver`가 있을 때만)가 살아남는 것을 찾아 시험 4개를 더했고, 각 변이가 새 시험 하나만으로 실패함을 확인했다. 정리 순서 중 fit 정리의 위치(`Terminal.dispose` 앞)는 시험하지 않는다(xterm이 dispose 때 addon도 정리한다). 경고 0 단언(`warnSpy`)은 대기 중인 write 콜백을 만들지 않아 TRP-004 회귀를 검출하지 못한다(브라우저 S04 몫).
...
- 패키지 경계: `src/package-boundary.test.ts`(의존 트리에 coincident·reflected-ffi 없음), `pnpm check-dist`, `pnpm smoke:pack`(react tarball 포함, tarball의 `react`·`react-dom`·`@xterm/xterm` peer 선언과 `@xterm/addon-fit` 정확 버전, 공개 진입점 import·소비자 `tsc --noEmit`·설치 트리 검사, `09-testing.md` 9.8). import는 모듈 평가만 하므로 `resolveTerminal()`의 `default` 폴백(TRP-063)은 어느 시험도 실행하지 않는다.
