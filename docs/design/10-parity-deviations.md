...
열린 읽기 위 배경 출력(RD-022b, 편차 4의 세부). 아래 세 건은 3.14와 나란히 측정하지 않았다(3.14 기본 REPL에는 대응 경로가 없고 `python -m asyncio`는 미측정).
...
56. **배경 `input()`의 `write`와 `readInput` 알림 사이에 REPL 줄 Enter가 끼면 stdin 읽기가 접두 없이 열린다.** 열린 REPL 줄 `>>> x = 41` 위에서 배경 `input("bg> ")`이 `bg> `를 쓰면 접두가 되어 `bg> >>> x = 41`이 되고, 두 알림 사이(다음 메시지 태스크 한 번)에 Enter가 오면 접두는 확정된 행에만 남고 stdin 읽기에 프롬프트가 없다: 화면 `["bg> >>> x = 41", "hello"]`(경합 없는 경로는 `[">>> x = 41", "bg> hello"]`). 원인: Enter 처리의 마이크로태스크가 `readInput` 메시지 태스크보다 먼저 돌아 도착 시 read-guard의 `replOpen`이 이미 false라 `inputDeferred`(`moveAbovePrefixToTail`)가 불리지 않는다(`04-stdin-input.md` 3.2). 고치지 않는다: 벤더가 Enter 시점 접두를 보관해 폴백으로 꼬리를 채우는 방식을 구현·시험했으나(RD-026 DELTA-04) 폴백에 닿으려면 `inputDeferred`의 호출 조건을 풀어야 하고, 풀면 접두가 남은 채 `input()` 코드를 Enter로 제출하는 정상 경로에서 접두가 stdin 프롬프트로 다시 그려지거나 `input("name: ")` 프롬프트를 덮을 수 있다(추정, 미시험). 경합과 정상 지연을 main이 구분할 수단이 없고 기존 read-guard 시험 2건의 기대값도 바꿔야 해 사용자가 편차 등록으로 확정했다(2026-09-25). 재개 조건: 그 둘을 구분할 수단(알림에 응답 수신 표지 등)이 생길 때. 이슈 `.scratch/repl-run-source-followups/issues/12-*.md`. 브라우저 셀 없음.
