...
결정: `crossOriginIsolated`가 거짓이면 시작 시 터미널에 경고를 내고 Ctrl+C·`input()` 없이 동작한다. 폴백 구현은 없다. dev·preview·정적 배포 모두 `Cross-Origin-Opener-Policy: same-origin`과 `Cross-Origin-Embedder-Policy: require-corp`를 보낸다(이전 구현은 dev에만 걸어 preview·빌드 산출물이 비격리였다).
