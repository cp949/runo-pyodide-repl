...
- `pnpm --filter demo e2e:<이름>` 단독 실행은 서버(주로 5173)가 이미 떠 있어야 한다
  (`e2e/README.md` "실행 전제", 31항목 목록은 같은 문서).
...
| `react-strictmode-check.mjs`(REPL `/`·실행창 `?view=runner`, `<StrictMode>` dev 전용, RD-024) | 화면당 5개 × 2 = 10개. S01 페이지 안 `Worker` 계측 `new` 2회 이상(이중 마운트 관측) + 살아 있는 worker 1(Playwright `worker`·`close` 이벤트와 계측 `new − terminate` 둘 다, 폴링), S02 `ready` 뒤에도 살아 있는 worker 1·`new` 수 불변, S03 `.xterm` 1개, S04 콘솔 warning `DisposableStore` 0건(정리 순서 회귀는 검출하지 못한다, `TRP-064`), S05 콘솔 warning·error·pageerror 0. Playwright 생성 이벤트는 terminate된 첫 worker를 못 봐(관측 1개, `new` 2회) 이중 마운트는 계측으로 본다 | 미실행(dev 전용) | **통과 10/10**(2026-09-25 RD-024 DELTA-06 dev L1 1회, `pageErrors` 0, `problemLogs` 0. 양성 대조: `useCoreHandle` cleanup의 `dispose()` 제거 → 실패) |
| `react-fit-check.mjs`(`/?fit=1`·`/?view=runner&fit=1`, RD-024) | 화면당 6개 × 2 = 12개. F01 초기 `cols` ≠ 기본 80(1280px 창, 실측 138), F02 창 640px → `cols` 감소(138 → 67), F03 창 1000px → 증가(67 → 107), F04 `x` 400자 출력의 줄바꿈 폭 = DOM 기하 `cols`(107), F05 REPL (a) 유휴 `print(1+1)` → `>>> print(1+1)` / `2` / `>>>`, 실행창 (b) `input()` 대기 중 창 500px(107 → 52) 뒤 `kim` 입력 → `이름: kim` / `안녕 kim`·결과 `ok`, F06 콘솔·pageerror 0. `cols`는 `.xterm-screen` 너비 ÷ 셀 너비(`.xterm-char-measure-element`)로 잰다(데모가 `Terminal`을 노출하지 않는다). `rows`는 판정 안 함(컨테이너 높이 auto) | 미실행(dev 전용) | **통과 12/12**(2026-09-25 RD-024 DELTA-06 dev L1 1회, `pageErrors` 0, `problemLogs` 0. `@xterm/addon-fit` 0.11.0 + `@xterm/xterm` 6.0.0의 첫 실제 브라우저 `cols` 변화 실측) |
